...
**운영 저장소는 Supabase PostgreSQL 하나이며 현재 앱 코드의 스키마 버전은 6입니다.** 계정·회원·두 티어·내전·경매·점수·업적과 Riot 조회 캐시·작업·요청 한도를 같은 DB에서 관리합니다. 최종 팀장 선택·참가 팀 카드 변경을 포함한 **고유 테스트 575개 전체 통과·예상 목록 대비 누락 0개·소스 135개 해시 일치**를 확인했습니다. 최초 병렬 실행은 보조 모듈 import 경로 오류 3개로 종료 코드 1이었으며, 제품 소스 변경 없이 `PYTHONPATH=tests`로 누락된 9개를 재검증해 통과한 뒤 전체 테스트 ID를 대조했습니다. 카드 변경 전 565개 결과는 과거 기준으로 보존하고, **실제 Cloud 배포는 아직 대기 상태**입니다. [최종 결과](test-artifacts/deployment-final-verification.json), [배포 준비 검증](DEPLOYMENT_VERIFICATION_REPORT.md), [이전 Riot 연동 보고](RIOT_API_REPORT.md)
...
1. Community Cloud에서 기존 앱의 저장소·브랜치·시작 파일을 확인합니다. 현재 소스의 **`streamlit_app.py`는 `app.py`와 같은 `roly.ui.run()`을 실행하는 호환 진입점**이므로 기존 시작 파일을 유지할 수 있습니다. 새 앱을 만들 때도 둘 중 하나를 지정합니다.
2. Python 버전은 **3.11**로 맞추고 이 프로젝트의 `requirements.txt`를 사용합니다. 과거 템플릿의 Python 3.14 설정과 `uv.lock`·`pyproject.toml`이 배포 의존성을 대신 선택하지 않도록 아래 [소스 반영 안내](#소스를-올릴-때-맞출-사항)를 확인합니다.
3. **Advanced settings → Secrets** 또는 앱 **Settings → Secrets**에 검수용 Supabase 연결 항목을 비공개로 입력하고, 다음 항목을 함께 설정합니다. 실제 비밀번호와 API 키는 문서·소스·ZIP에 넣지 않습니다.
...
5. 비공개 CLI로 최초 관리자를 준비한 뒤 검수 앱에서 ‘검수용’ 배지와 체험 메뉴 부재를 확인합니다. 다른 브라우저로 실제 개인 가입→승인→로그인→카카오톡 명단 확정→팀장 입찰을 검수합니다. 제공된 체험 계정을 배포용으로 사용하지 않습니다.
6. 검수가 끝나면 사용할 DB 프로젝트를 다시 확인하고 `[app] environment = "production"`으로 전환합니다. 모드만 바꿔도 DB가 복사·초기화되지는 않습니다.

**아직 수행하지 않은 작업:** 실제 최초 관리자 생성, GitHub 업로드, Community Cloud 배포입니다. 앱 스키마의 수동 백업·별도 복원 검증 도구와 워커 프로세스 복구 리허설은 추가했지만, 예약 백업·운영 DB 복원과 Cloud 휴면 중 독립 마감은 완료하지 않았습니다. 계정 비밀번호 복구는 운영진이 본인 확인 후 일회용 코드를 발급하는 방식입니다. 월 운영 예산은 0원이며 첫 외부 검수 이름은 `rolymoly-test`입니다.

다음 세 문단의 16단계·12단계·219개는 최초 PostgreSQL 이식 시점의 과거 검증입니다. 현재 개인 가입·회원 티어·일반내전 추가 검증과 구분합니다.
...
PostgreSQL 전환 후 최종 전체 회귀 테스트는 **219개 통과·427.619초·종료 코드 0**이며 검사 전후 Python 소스 63개 해시가 일치했습니다. 2026-09-07~08의 Chrome 6계정 리허설과 당시 SQLite 기준 자동 테스트 166개 결과는 [REHEARSAL.md](REHEARSAL.md)에 별도 과거 검증 이력으로 보존합니다.
...
실제 최초 관리자는 아직 생성하지 않았습니다. 배포자가 비공개 터미널에서 다음 명령을 실행해 아이디·표시 이름·앱 비밀번호를 입력해야 합니다. 비밀번호 입력은 표시되지 않으며 기존 관리자가 있거나 비밀번호 확인이 다르면 계정을 만들지 않습니다.
...
경매 워커 복구는 동결한 소스의 실제 PostgreSQL QA에서 **9단계·66.032초 통과**했습니다. 워커가 없으면 만료된 경매도 정산되지 않는 것을 확인한 뒤, 새 프로세스가 브라우저 없이 기존 낙찰을 한 번만 정산하고 다음 선수를 최소 3초 뒤 여는 것을 확인했습니다. 일시정지는 재시작 후에도 유지됐으며 두 워커의 동시 실행에서 중복 차감은 없었습니다. 생성한 QA 스키마와 네 워커 프로세스는 모두 정리했습니다. 근거: `test-artifacts/deployment-recovery-remote.json`.
...
| --- | --- | --- | --- |
| 최초 관리자 | PostgreSQL 공개 생성 차단과 비공개 CLI 구현·테스트 완료, 실제 계정 0개 | 배포자가 `create_admin.py`로 실제 계정 생성 | 해당 계정으로 검수 앱 로그인 가능 |
...
| Cloud 배포·업데이트 | Python 3.11·고정 requirements와 `streamlit_app.py` 호환 진입점 준비, 원격 업로드·배포 없음 | 시작 파일·Secrets 설정, 여러 기기 검수와 재시작·롤백 절차 확인 | Cloud에서 전체 흐름·재연결·운영 자료 보존 확인 |
...
- 모바일 입찰 화면에서는 현재 선수·최고 입찰·타이머·입찰 버튼을 먼저 배치하고 팀 현황을 접거나 뒤로 보냅니다. 직전 390×844 검수에서는 입찰 버튼이 약 1,488px 아래에 있었습니다. PC에서 통과한 화면 배치만으로 모바일 입찰 사용성이 확보되지는 않습니다.
...
선택한 구성은 **Streamlit Community Cloud + Supabase Free의 PostgreSQL**입니다. Supabase 연결은 완료했고 Community Cloud 배포는 남아 있습니다. 회원·계정·내전·입찰·기록은 이 DB에서 관리하며 Google Sheets 연결·동기화·기존 자료 가져오기는 운영 범위에서 제외합니다. 월 예산 0원의 무료 범위에서 검수하며 유료 추가 기능·요금제를 적용하지 않습니다. 플랜의 저장 공간·전송량·프로젝트 한도는 배포 시 공식 자료에서 확인합니다. [Community Cloud 안내](https://docs.streamlit.io/deploy/streamlit-community-cloud), [Supabase 요금표](https://supabase.com/pricing)

| 역할 | 구성과 진행 상태 |
| --- | --- |
| 화면·주소 | Community Cloud의 검수 앱 `rolymoly-test`, 시작 파일 `streamlit_app.py` 또는 `app.py`. 목표 URL은 `rolymoly-test.streamlit.app`이며 실제 사용 가능 여부는 배포 설정에서 확인 |
...
`rolymoly-test`는 사용자 지정 검수 이름입니다. 기존 앱을 유지한 채 새 앱의 App URL을 정할 수 있으며, 해당 이름의 예약·사용 가능 여부와 GitHub 저장소/브랜치 연결은 아직 확인하지 않았습니다. [새 앱 배포](https://docs.streamlit.io/deploy/streamlit-community-cloud/deploy-your-app/deploy), [주소 변경·중복 확인](https://docs.streamlit.io/deploy/streamlit-community-cloud/manage-your-app/app-settings)
...
Supabase 프로젝트와 DB 연결은 준비되었습니다. 외부 배포에는 GitHub/Community Cloud 계정과 앱 연결 설정이 필요합니다.
...
아래 원격 저장소 정보는 **2026-09-07 읽기 조회 당시의 기록**이며, 이번 문서 갱신에서 원격 HEAD나 실제 Community Cloud 연결 설정을 새로 확인한 것은 아닙니다.

## 원격 저장소에서 확인한 내용

Git의 `ls-remote`와 인증 없는 GitHub REST API로 공개 메타데이터와 파일 구조를 읽었습니다.
...
| 새 앱 파일 | 원격에는 아직 `app.py`, `roly/`, `app_pages/`, `requirements.txt`가 없음 |
| 현재 작업 폴더 | Git 저장소로 초기화되어 있지 않아 원격 연결 정보 없음 |
| 기존 공개 앱 연결 | `rolymoly.streamlit.app`이 이 저장소·브랜치·시작 파일과 연결되어 있는지는 확인하지 못함 |

원격 README의 앱 링크도 기본 템플릿 주소이고, GitHub Deployments 조회 결과는 비어 있었습니다. 이것만으로 Community Cloud 배포 유무를 판단할 수는 없습니다. 실제 연결 정보는 소유자의 Streamlit Community Cloud 관리 화면에서 확인해야 합니다.
...
현재 소스는 `app.py`와 호환 파일 `streamlit_app.py` 모두에서 같은 앱을 실행합니다. **기존 Cloud 시작 파일이 `streamlit_app.py`이면 이름을 유지하고 해당 파일을 이번 소스로 교체할 수 있습니다.** 2026-09-07에 확인한 저장소 후보는 `lsh920306/rolymoly`의 `main`이며, 실제 연결은 소유자의 Cloud 설정에서 확인합니다. 배포 Python은 **3.11**, 의존성은 이번 `requirements.txt`에 맞춥니다. [공식 배포 안내](https://docs.streamlit.io/deploy/streamlit-community-cloud/deploy-your-app/deploy)

기존 템플릿 파일을 그대로 남기고 새 파일만 추가하면 환경이 섞입니다. 템플릿의 `uv.lock`·`pyproject.toml`·Python 3.14 설정이 새 `requirements.txt`와 Python 3.11을 대신 선택하지 않도록 소스 반영 시 정리합니다. `streamlit_app.py`는 제거 대상이 아니라 새 호환 진입점으로 교체할 파일입니다. 원격 저장소 반영과 실제 Cloud 설정 확인은 아직 수행하지 않았습니다. [의존성 파일 우선순위](https://docs.streamlit.io/deploy/streamlit-community-cloud/deploy-your-app/app-dependencies)
...
| 최초 관리자 설정 | PostgreSQL 공개 생성 폼은 차단했습니다. 배포자가 비공개 CLI로 최초 관리자를 준비한 뒤 Cloud에서 로그인을 확인해야 합니다. 이후에는 승인된 개인계정에 운영 역할을 부여합니다. 개발용 SQLite의 초기 설정 폼은 유지됩니다. |
...
| 동시 접속 검증 범위 | 과거 SQLite·Chrome 리허설에 더해 실제 PostgreSQL 4명 경쟁 입찰과 전체 16단계를 확인했습니다. Cloud와 여러 실제 기기의 인터넷 지연·부하는 아직 측정하지 않았습니다. |
...
새 검수 앱에서 연결·로그인·권한·경매·결과를 확인한 뒤 공개 범위를 정합니다. 현재 PostgreSQL 통합 완료는 GitHub 업로드나 Cloud 앱 공개를 의미하지 않습니다.
...
기존 Community Cloud 앱에 이 저장소가 이미 연결되어 있다면 해당 브랜치의 변경은 자동 반영될 수 있습니다. 따라서 연결된 시작 파일·Python 버전·데이터 보관 방식을 확인한 뒤 실제 업로드를 진행해야 합니다. [GitHub 변경 자동 반영 안내](https://docs.streamlit.io/deploy/streamlit-community-cloud/manage-your-app)
