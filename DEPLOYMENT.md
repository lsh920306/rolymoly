...
2026-09-09에 확인한 상태입니다. **사용자 승인 후 첫 배포 `a3a091d`와 연결풀·전체 팀 팝업 개선본 `cf92f09`를 Cloud에 반영했습니다.** Cloud의 빌드·로그인·전체 페이지와 전체 팀 팝업의 720px 너비·26px 제목·4팀 스크롤 없는 배치를 확인했습니다. 이후 입찰 금액 덮어쓰기·팝업 반복 열림 수정본의 **고유 테스트 606개가 모두 통과**했습니다. 해당 로컬 브라우저에서 10 P 입찰·낙찰·1,000→990 P 차감 및 다른 Cloud 접속의 동기화를 확인했습니다. 그 뒤 추가한 경매 3단 배치·원본 로고의 영향 검증 35개도 통과했으며, 최종 재배포와 Cloud 직접 조작 검수는 남아 있습니다.

최신 화면 변경은 **왼쪽 팀 현황 / 가운데 경매·입찰 / 오른쪽 입찰 기록**의 3단 배치와 사용자 원본 로고 [static/rolymoly-logo.png](static/rolymoly-logo.png)의 200px 너비 표시입니다. 이 변경은 아래 606개 전체 검증 이후에 추가했으므로 관련 검증을 분리합니다. 로컬 실제 브라우저 확인과 자동 영향 검증 35개를 완료했고, 최종 Cloud 반영은 아직 남아 있습니다.
...
| 검수·배포 브랜치 | 첫 배포 `a3a091d`와 중간 개선본 `cf92f09`의 Cloud 반영 완료. 이후 입찰 금액·팝업 추가 수정본은 최종 커밋·재배포 전 |
...
| Cloud 확인 완료 | 비로그인·개인회원·관리자 전체 페이지와 권한 동선, 720px 전체 팀 팝업·26px 제목·4팀 스크롤 없는 표시, 다른 접속에서 10 P 낙찰 결과 동기화 |
| 보존한 공유 자료 | 회원 22명·계정 23개·저장된 Riot 프로필 20개와 기존 준비 경매 1번 보존 |
| Cloud 경매 추가 검수 | 별도 임시 이벤트 2번 유지. 최종 수정본 재배포 후 Cloud 직접 입찰·팝업 닫기·일시정지/재개·초기화 확인 예정 |
| 최신 수정 소스 | 입찰 금액·팝업 경합 수정 후 경매 3단 배치와 사용자 원본 로고 200px 표시 추가. 최종 재배포 전 |
...
1. 현재 소스의 **`streamlit_app.py`는 `app.py`와 같은 `roly.ui.run()`을 실행하는 호환 진입점**입니다. 승인된 `main`의 첫 배포 `a3a091d`와 중간 개선본 `cf92f09`를 이 진입점으로 배포했습니다. 이후 수정도 Cloud가 연결한 `main`의 커밋과 실제 빌드를 함께 확인합니다.
...
| Cloud 배포·업데이트 | 승인된 첫 배포와 `cf92f09` 개선본 Cloud 반영, 전체 페이지·팝업·낙찰 결과 동기화 확인. 추가 조작 수정본 고유 606개 통과 | 최종 수정본 재배포, Cloud 직접 입찰·닫기·재개·초기화·여러 기기·재시작·롤백 확인 | Cloud에서 전체 흐름·재연결·운영 자료 보존 확인 |
...
현재 소스는 `app.py`와 호환 파일 `streamlit_app.py` 모두에서 같은 앱을 실행합니다. **Cloud 시작 파일 `streamlit_app.py`를 유지하여 첫 배포 `a3a091d`와 중간 개선본 `cf92f09`를 반영했습니다.** 최종 추가 수정본도 같은 진입점으로 재배포하며 Python **3.11**과 이번 `requirements.txt`를 유지합니다. [공식 배포 안내](https://docs.streamlit.io/deploy/streamlit-community-cloud/deploy-your-app/deploy)
...
| 동시 접속 검증 범위 | 실제 PostgreSQL 경쟁 입찰·워커 복구·연결풀 격리, 최신 로컬의 10 P 입찰·낙찰·10 P 차감 및 다른 Cloud 접속 동기화를 확인했습니다. 최종 추가 수정본 재배포 후 Cloud 직접 조작을 다시 검수합니다. 여러 실제 기기의 인터넷 지연·부하 보장값은 아직 없습니다. |
...
Cloud 검수 앱은 중간 개선본 `cf92f09`까지 반영했고, 연결·로그인·페이지 권한과 전체 팀 팝업 표시를 확인했습니다. 추가 수정 후 최신 로컬 브라우저에서 팝업 닫기 1회 뒤 닫힘 유지 → 재개 → 10 P 입찰 접수 → 낙찰 → 1,000→990 P 차감을 확인했고 다른 Cloud 접속에도 결과가 동기화됐습니다. PostgreSQL의 입찰·낙찰 각 1건과 선수 배정, 회원·계정·Riot·이벤트 1번 보존을 읽기 전용으로 대조했습니다. 브라우저 관찰 시각과 DB 시각은 실제 요청 전송에 맞춘 측정값이 아니므로 응답 지연 수치로 사용하지 않습니다. [입찰 DB 대조](test-artifacts/browser-live-bid-postgres-20260909.json)

입찰 금액·팝업 수정본의 고유 606개 검증과 이후 3단 배치·로고 영향 검증 35개를 완료했습니다. 최종 재배포 후 Cloud 직접 입찰·닫기·재개·초기화를 확인합니다. 임시 이벤트 2번의 정리 스크립트는 읽기 전용 드라이런까지만 완료했고, 별도 검수 완료 지시 전에는 삭제하지 않습니다.
