...
from roly.ui import context, deferred_csv, heading, perform, require_staff
        ...
        if not members:
            ...
            if target_id is not None:
                ...
                if target["status"] == "KICKED":
                    ...
                    membership_panel = st.expander("회원 탈퇴 처리", key=f"admin_membership_panel_{target_id}", on_change="rerun")
                    if membership_panel.open:
                        with membership_panel:
            ...
            adjustment_history = st.expander("최근 점수 보정 내역", key="admin_adjustment_history", on_change="rerun")
            if adjustment_history.open:
                with adjustment_history:
...
if policy_tab.open:
    with policy_tab:
        ...
        policy_history_panel = st.expander("점수 정책 변경 내역", key="admin_policy_history", on_change="rerun")
        if policy_history_panel.open:
            with policy_history_panel:
                st.dataframe([
...
if audit_tab.open:
    with audit_tab:
        ...
        if not audit_rows:
            ...
            st.download_button(
                "표시된 변경 기록 CSV 다운로드", data=deferred_csv(core, audit_display, token=token, admin=True),
                file_name="rolymoly_audit.csv", mime="text/csv", icon=":material/download:", on_click="ignore",
            )
