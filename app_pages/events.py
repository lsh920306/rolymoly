...
from roly.ui import (
    ...
    can_edit, context, deferred_csv, event_picker, heading, perform, team_cards,
)
...
with selected_tab:
    ...
    bracket_tab, team_tab, history_tab = st.tabs([f"{kind_label} 결과", f"{kind_label} 명단", "전체 경기 이력"],
                                               key=f"events_detail_tab_{kind}", on_change="rerun")
            ...
            if event is not None:
                if event["status"] in ("DRAFT", "RECRUITING", "CAPTAIN_SELECTION", "TEAM_BUILDING", "AUCTION_READY", "AUCTION", "BRACKET_SETUP"):
                    ...
                    if closed_unfinished:
                        ...
                        st.download_button(f"{kind_label} 결과 CSV 다운로드", deferred_csv(core, lambda games=event["games"]: fixture_rows(games)),
                            file_name=f"rolymoly_event_{event_id}_results.csv", mime="text/csv", icon=":material/download:", on_click="ignore")
                    else:
        ...
        with team_tab:
            if event is not None:
                ...
                if editable and event["kind"] == "NORMAL" and event["status"] == "READY":
                    roster_edit_panel = st.expander("카카오톡 확정 명단 수정·재편성", key=f"events_roster_editor_{event_id}", on_change="rerun")
                    if roster_edit_panel.open:
                        with roster_edit_panel:
                ...
                st.download_button(f"{kind_label} 명단 CSV 다운로드", deferred_csv(core, roster_rows), file_name=f"rolymoly_event_{event_id}_roster.csv", mime="text/csv", icon=":material/download:", on_click="ignore")
                event_audit_panel = st.expander(f"{kind_label} 변경 기록", key=f"events_audit_{event_id}", on_change="rerun")
                if event_audit_panel.open:
                    with event_audit_panel:
        ...
        with history_tab:
            ...
            if not history_games:
                ...
                if history_id is not None:
                    ...
                    st.download_button("선수별 경기 기록 CSV 다운로드", deferred_csv(core, score_rows), file_name=f"rolymoly_game_{history_id}_players.csv", mime="text/csv", icon=":material/download:", on_click="ignore")
