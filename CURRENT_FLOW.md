...
운영의 기준 저장소는 Supabase PostgreSQL 하나입니다. 계정·가입 신청·회원·참가 확정 명단·팀·입찰·잔액·경기·전력점수·우승 업적·감사 기록을 함께 저장합니다. 현재 실행 코드와 의존성에는 구글시트 연결이 없습니다. 과거 추출 문서의 구글시트 설명은 참고 코드의 설명입니다.

카카오톡은 접수 창구이며 확정된 명단은 DB에 등록합니다. Google Sheets 연결·동기화·기존 자료 가져오기는 운영 범위에서 제외합니다. CSV 다운로드는 DB 자료를 파일로 확인하는 기능이며 별도 운영 저장소가 아닙니다. 체험 공간의 SQLite는 예시 자료를 격리하는 용도이며 운영 자료와 합치지 않습니다. 현재 체험 데이터 버전은 **5**로, 제공된 Riot ID 22명과 성공한 21명의 공개 조회 스냅샷을 사용합니다. 체험 점수·클랜 티어·경기는 예시이며 운영 PostgreSQL 스키마 버전 **6**과도 별개입니다. DB 연결 설정이 잘못됐을 때 다른 저장소로 조용히 넘어가지 않습니다.
...
회원 목록은 각 회원 행의 ‘프로필 상세보기’ 버튼으로, 라운지는 회원 카드 클릭으로 같은 프로필 페이지에 이동합니다. 관리자는 상세 페이지의 ‘회원 정보 수정’을 사용할 수 있습니다. 운영 관리의 회원 수정도 같은 저장 기능을 사용합니다. 일반 회원은 조회하고 운영진에게 변경을 요청합니다. 운영자는 DB 관리 사이트나 시트를 직접 편집할 필요가 없습니다.

| 데이터 | 저장·수정 기준 |
| --- | --- |
| 로그인 아이디·회원 ID | 계정과 회원을 연결하는 고정 기준. Riot ID를 변경해도 로그인 아이디·연결 ID는 유지 |
| Riot ID·주/부 포지션 | 가입 신청 때 입력. 승인 전에는 본인 신청에서, 승인 후에는 관리자가 수정 |
| 클랜 티어 | 운영진의 내부 평가. 관리자만 수정하는 32자 이내 선택적 명칭 |
| 현재 티어·LP | Riot API의 한국 서버 솔로랭크. 연결 전에는 가입·관리자 입력값을 사용하며, API 갱신 후에는 조회로 갱신 |
| 프로필 자유랭크 | 같은 Riot 응답에서 자유랭크 티어·LP·승패를 별도로 저장. 솔로랭크 평가값을 덮어쓰지 않음 |
...
| 신청 메시지 | `application_notes`. 신청자가 승인 전에 작성·수정하고 운영진이 확인 |
...
Riot 정보는 같은 Supabase DB의 `riot_profiles`에 저장합니다. 개인 프로필에서 ‘갱신하기’를 누르거나 일반내전·경매 준비 화면에서 한 회원을 선택한 뒤 ‘이 회원 갱신’을 누를 때만 요청합니다. 화면 진입·검색·자동 재실행으로 새 조회를 예약하지 않으며, 명시적으로 예약한 작업은 제한 대기 후 이어갑니다. 경매 선수 카드의 현재 티어·주력 챔피언은 최신 조회 캐시이고, 클랜 티어·전력은 확정 명단 기준입니다. 경기 기록의 당시 티어는 API 조회로 덮어쓰지 않습니다.
...
`riot_jobs`는 단계별 조회와 재시도를 보존하고, `riot_rate_hits`·`riot_rate_cooldowns`는 키별 호출 수와 대기 시간을 함께 관리합니다. 1초 20회·120초 100회 이내로 제한하며 429 응답에 지정된 시간 동안 기다립니다. API 요청은 경매 화면이나 입찰 트랜잭션에서 실행하지 않습니다. 조회 실패 시 마지막 정상 정보를 유지합니다. Riot ID 변경은 이전 캐시와 작업을 폐기하며 로그인 연결·전력·기존 기록을 바꾸지 않습니다. 코드의 스키마 버전은 6이며 이전 검증 보고와 이번 추가 변경은 [Riot 연동 보고](RIOT_API_REPORT.md)에서 구분합니다.
...
1. 회원이 가입 신청에서 로그인 아이디·비밀번호·Riot ID·주/부 포지션과 선택적 현재 티어·LP를 입력합니다. 계정·회원 정보·신청서를 한 번에 저장합니다.
...
상단에는 현재 → 다음 선수와 오른쪽 스피커·음량 조작부를 두고, 왼쪽 화살표로 모든 팀을 순환 조회합니다. 다른 팀을 보고 있어도 입찰은 본인의 고정 팀으로만 접수됩니다. 전체 팀 현황은 배정 포지션별로 표시하며 같은 포지션 선수가 여럿이어도 누락하지 않습니다. **금액 초기화**는 입력값만 현재 최고가로 되돌리고 DB 입찰·잔액·마감을 변경하지 않습니다.

**경매 초기화**는 설정이 저장된 준비 상태, 진행 중, 대진 생성 전 완료 상태에서 주최자·관리자가 환불·잔액 미리보기와 사유를 확인한 뒤 실행합니다. 참가자·고정 팀장·시작 예산·입찰 시간은 보존하고, 낙찰 배정을 되돌려 환불한 뒤 선수 순서를 새로 섞어 시작 대기로 돌아갑니다. 대진·실경기·우승 보상이 생긴 뒤에는 차단합니다. 초기화·취소 전 입찰은 별도 접힌 기록에 보존하고 현재 입찰 목록과 효과음 대상에서 제외합니다. [화면·정책 대응표](AUCTION_REFERENCE_MAPPING.md)
...
| 동시·중복 입찰 | 서버가 권한·대상·금액·잔액·정원·마감·요청 ID를 같은 저장 작업에서 검사합니다. 같은 금액 경쟁은 먼저 유효하게 저장된 한 건만 접수합니다. |
...
| 입찰 반응 속도·시간 표시 | 구현. 진행 화면 0.5초 갱신, DB 묶음 조회, 지연된 동일 마감 응답의 시간 증가 방지. 실제 DB API 성능 비교 완료, 브라우저 표시 지연 검수는 별도 | [진행 화면](roly/auction_ui.py), [DB 조회](roly/live_auction.py), [측정 보고](DB_IMPROVEMENTS_REPORT.md) |
...
| 운영 DB 백업·복원 | 자동화와 실제 복원 검증 미완료 | [배포 과제](DEPLOYMENT.md) |
| GitHub·Cloud 배포, 여러 기기 검수 | 미완료. 합성 DB·AppTest·서버 리허설과 구분 | [검증 범위](QA_SCENARIOS.md), [배포 상태](DEPLOYMENT.md) |
...
이전 v5판은 전체 회귀 366개를 한 번의 실행에서 모두 통과했고, 성능 개선 후 원격 리허설 58단계도 다시 통과했습니다. 소스·스타일·설정 96개 파일(그중 Python 88개)의 실행 중 해시 변경은 없었습니다. **현재 v6와 최신 경매 UI의 검증은 별도**이며 [시나리오별 QA 보고](QA_SCENARIOS.md)와 [Riot 연동 보고](RIOT_API_REPORT.md)를 기준으로 확인합니다. 과거 `test-artifacts/member-db-verification.json`이나 브라우저 리허설을 현재 소스의 전체 통과·실제 브라우저 검증으로 대체하지 않습니다. 검수는 합성 자료와 별도 QA DB를 사용하며 GitHub·Cloud 배포와 구분합니다.

진행 중 경매는 0.5초 간격 Streamlit fragment 자동 갱신입니다. 준비 화면과 전체 팀 보기 창의 상태 조회는 1초 간격을 유지합니다. 마감은 서버가 판단하며 웹소켓 이벤트 푸시 방식으로 교체한 것은 아닙니다. 화면 자료를 모두 조회한 뒤 남은 시간을 계산하고 렌더에 소요된 시간을 차감합니다. 같은 선수·마감시각·단계의 늦은 응답이 도착해도 브라우저의 남은 시간이 증가하지 않습니다. PostgreSQL 조회와 트랜잭션 초기 설정은 같은 연결에서 묶어 전송하며 권한이나 입찰 결과를 캐시하지 않습니다. 배포 환경의 지연·여러 기기·모바일 배치·탭 복원과 음량 동작은 실제 브라우저에서 추가 확인해야 합니다. Python 워커는 앱 프로세스가 실행 중일 때 동작하므로 Cloud 절전 중의 독립 마감 보장, 운영 DB 백업·복원 검증도 별도로 필요합니다.

현재 PostgreSQL 스키마 버전 **6**은 개인 가입·비밀번호 복구·회원 티어·경기 당시 정보·수기 보정 영수증과 v5의 신청 메시지·내전 생성 영수증을 유지합니다. 여기에 `current_tier_source`와 `riot_profiles`·`riot_jobs`·`riot_rate_hits`·`riot_rate_cooldowns`를 추가합니다. 기존 내부 메모·회원·계정·원장·기록을 보존하는 구조 추가이며 다른 DB나 시트의 자료 이관이 아닙니다. 과거 v4→v5 검수는 보존하고, v5→v6 검수는 새 QA 스키마에서 별도로 확인했습니다. [v6 PostgreSQL 리허설](test-artifacts/final-ui-riot-remote.json)
