...
def profile_header(member, profile):
    ...
    rank = "한국 서버" if profile else (
        "Riot API · 상세 정보 없음" if member.get("current_tier_source") == "riot" else
        "Riot 조회 전 · 수기 입력" if member.get("current_tier") else "아직 Riot 조회 정보가 없습니다.")
